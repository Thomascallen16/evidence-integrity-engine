...
function validateAudit(events: AuditEvent[], knownIds: Set<string>) {
  ...
  for (const event of events) {
    if (!event.id || !event.timestamp || !event.entityType || !event.entityId || !event.action || !event.summary?.trim()) {
      issues.push(`Audit event ${event.id || "(missing id)"} is structurally incomplete.`);
    }
    if (!knownIds.has(event.entityId)) issues.push(`Audit event ${event.id} references unknown entity ${event.entityId}.`);
  }
  for (let i = 1; i < events.length; i++) {
    if (String(events[i].timestamp) < String(events[i - 1].timestamp)) {
      issues.push("Audit events are not in chronological order.");
      break;
    }
  }
  return issues;
...
/**
 * Deterministic rule: a source alone never promotes a claim to FACT.
 * FACT requires source-anchored exact evidence, an explicit SUPPORTING link,
 * no unresolved structural ambiguity, and no rejected/needs-review supporting evidence.
 */
export function evaluateIntegrity(input: IntegrityInput): IntegrityFinding {
  ...
  const verifications = input.verificationRecords ?? [];
  const audits = input.auditEvents ?? [];
  ...
  const duplicateSourceIds = duplicateIds(sources.map((s) => s.id));
  const duplicateEvidenceIds = duplicateIds(evidenceItems.map((e) => e.id));
  const duplicateLinkIds = duplicateIds(links.map((l) => l.evidenceId));
  ...
  const malformed = evidenceItems.filter((e) => !e.id || !sourceIds.has(e.sourceId) || !e.exactText?.trim());
  if (malformed.length) unknowns.push("One or more evidence items are missing a valid identifier, source anchor, or preserved exact text.");

  const orphaned = links.filter((l) => !evidenceIds.has(l.evidenceId));
  if (orphaned.length) unknowns.push("One or more evidence links point to evidence that is not present in the record.");

  const invalidRelationships = links.filter((l) => l.relationship !== "SUPPORTING" && l.relationship !== "CONTRARY");
  if (invalidRelationships.length) unknowns.push("One or more evidence links contain an invalid relationship.");
  ...
  const usableContrary = contrary.filter((e) => !rejectedEvidenceIds.includes(e.id));
  ...
  const base = {
    supportsClaim: usableSupporting.length > 0,
    supportingEvidenceIds: supporting.map((e) => e.id),
    contraryEvidenceIds: contrary.map((e) => e.id),
    missingEvidence,
    unknowns,
    reasons,
    rejectedEvidenceIds,
