...
(function(global){
  ...
  function validEvidence(input){const sourceIds=new Set(input.sources.map(s=>s.id));return input.evidence.filter(e=>sourceIds.has(e.sourceId)&&e.exactText.trim().length>0)}
  function linkedEvidence(evidence,links,relationship){const byId=new Map(evidence.map(e=>[e.id,e]));return links.filter(l=>l.relationship===relationship).map(l=>byId.get(l.evidenceId)).filter(e=>Boolean(e))}
  function evaluateIntegrity(input){
    const reasons=[],unknowns=[],missingEvidence=[],sources=input.sources??[],evidenceItems=input.evidence??[],links=input.evidenceLinks??[];
    ...
    const malformed=evidenceItems.filter(e=>!sourceIds.has(e.sourceId)||!e.exactText?.trim());
    if(malformed.length)unknowns.push("One or more evidence items are missing a valid source anchor or preserved exact text.");
